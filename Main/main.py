...
def checkAPPUI(device, appActName, appActMain):
    # ---检测是否进入应用
    ...
    for activityName in activityNames:
        if currentActivity.find(activityName) >= 0:
            print("[设备]-" + device + " 设备处于app界面"+str(count)+"........")
            return
    ...
    while resActivity.find(configProps.get(appActMain)) < 0:
        time.sleep(1)
        ADBAction.startActivitySingleDevice(configProps.get(appActMain), device)
...
def app_qutoutiao(device):
    # -----------------------------开始---------------------------------
    # ---打开app界面
    maintain_app_ui(device, configProps.get("qttActMain"))
...
# ---微鲤看看
def app_weili(device):
    # -----------------------------开始---------------------------------
    # ---读取相关的配置信息
    MainActivity = configProps.get("weiliActMain")
    # ---打开app界面
    maintain_app_ui(device, MainActivity)
    # ---此延迟时间根据手机性能 自行配置等待app加载时间 后期可以考虑使用opencv做验证
    time.sleep(10)
    # --------------------------CV检测广告-------------------------------
    # ---截取初始界面 用于检测广告
    ...
    # ---加载广告图片模板
    # imgAppMain = cv2.imread("qutoutiao/img/main.jpg", 0)
    imgAd1 = cv2.imread("weili/img/ad1.jpg", 0)
    # imgAd2 = cv2.imread("qutoutiao/img/ad2.jpg", 0)
    # ---opencv检测当前状态 之前利用uiautomator的xml 被app封锁 抓取率不高
    # ---匹配当前状态 1，广告 2，主界面 3，其他干扰
    # ---将模板读取灰度
    ...
    if min_val < 0.01:
        print("[设备]-" + device + " 出现广告1，处理中.........")
        ADBAction.clickXY(device, 529, 1399)
    ...
    app_time = time.time()
    # ---run之前，无论如何强制返回app主界面 保证正确率
    ADBAction.clickXY(device, 325, 1857)
    time.sleep(1)
    # ---签到
    ADBAction.clickXY(device, 80, 162)
    time.sleep(2)
    ADBAction.clickXY(device, 536, 307)
    ADBAction.actionBack(device)
    # ---定时器任务集合
    # ---此为app定时奖励 此app不存在
    # schedule.every(2).minutes.do(rewardClick, device)
    # ---定时检测当前是否还在app界面
    schedule.every(3).minutes.do(checkAPPUI, device, "weiliActs", "weiliActMain")
    # ---定时忘记了。。。。。
    # schedule.every(30).minutes.do(refresh, device)
